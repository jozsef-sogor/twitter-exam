...
const RegisterModal = defineAsyncComponent(() =>
  import("./components/modals/RegisterModal.vue")
);
const ForgotPasswordModal = defineAsyncComponent(() =>
  import("./components/modals/ForgotPasswordModal.vue")
);
const ResetPasswordModal = defineAsyncComponent(() =>
  import("./components/modals/ResetPasswordModal.vue")
);
...
const friendSuggestions = [
  {
    src:
      "https://sogor98.eu.pythonanywhere.com/assets/img/default-profile-image.jpeg",
    name: "Elon Musk",
    handle: "@teslaBoy",
  },
  {
...
<template>
  <div class="flex w-full h-screen xl:container">
    ...
    <main class="w-full h-full overflow-y-scroll md:w-1/2">
      <header
        class="sticky top-0 flex items-center justify-between w-full px-5 py-3 bg-white border-b border-lighter"
      >
    ...
    <component
      ...
      v-if="modalToShow && !auth"
      @submit-form="handleModalSubmit"
